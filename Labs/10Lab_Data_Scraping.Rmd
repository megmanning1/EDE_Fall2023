...
#3 Construct a dataframe from the values - JUST GETTING NAs, need solutions...
...
scrape.it <- function(the_year){
  #Get the proper url
  #ifelse - if the year is 2022 then we use, if not then we do paste0
  the_url <- ifelse(
    the_year=='2022',
    'https://www.eia.gov/electricity/state/',
...
df_years <- lapply(seq(2017,2019), scrape.it) %>%
  bind_rows()
