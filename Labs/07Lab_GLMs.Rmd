...
```{r}
#Exercise 1:
#geom_density of ozone divided by year - ozone (y), year (x)
#alpha adds transparency
#if you have a column that's a number, it won't be able to identify it as a factor
Ozone.density <- ggplot(EPAair, aes(x = Ozone, 
                                    fill = as.factor(Year))) +
  geom_density(alpha = 0.5)+
  scale_fill_manual(values = c("darkgreen", "darkblue"))+
  labs(title = "Ozone Divided by Year",
       x = "Ozone AQI value", 
       y = "Density",
       fill="Year")
print(Ozone.density)
...
#a and b represent max value for each year
...
#geom_violin 
#can move "a" and "b" around to max, min, mean whatever you want it to show 
Ozone.violin <- ggplot(EPAair, aes(x=as.factor(Year),
                   y=Ozone))+
...
#could also say lm(data = EPAair, PM2.5 ~ Ozone)

summary(PM25.regression)
#r-squared says Ozone only explains 13% of PM2.5
plot(PM25.regression)
...
#if ask for variable BY variable (left of by is y-axis) -- ex. temp BY depth means temp = y and depth = x
PM2.5byOzone.plot <- ggplot(EPAair,
                            aes(x = Ozone,
                                y = PM2.5)) +
  geom_point() +
...
#what I tried during lab...
ggplot(EPAair, aes(z=Ozone, y=PM2.5))+
  ylim(0,100)+
  geom_point()+
  geom_smooth(method = "lm")
