...
#Read packages 
#doing it on one line separated by semi-colons
library(tidyverse);library(lubridate); library(here)

#Read in data, don't convert strings to factors
#here() - is a good function to find your working directory and easy way to load data
#didn't read in stringsAsFactors (so you can see that there are characters in the environment tab)
sites <- read.csv(here('Data/Raw/NWIS_SiteFlowData_NE_RAW.csv'))
dim(sites)

#Convert date column to date object
#another function to use to convert characters to dates --> yearsmonthday_hoursminutesseconds (ymd_hms)
#POSIXct is an internal format
...
#4 = Wednesday (Sunday, Monday, Tuesday, Wednesday)
...
#tells you which days the data was collected
unique(sites$dow)

#Summary - is it meaningful? --> not really because not all values are numbers (as in 4 = Wednesday so taking mean doesn't really make sense)
summary(sites$dow)
...
#Convert to a factor - overriding column values as factors 
...
#Summary - is it meaningful? -- will give you list of unique values and counts which is useful here because we're grouping the values of same numbers together (as in not numerically computing mean, seeing how many of those numbers are present)
summary(sites$dow)
...
#this is useful because maybe we want to see all days of the week regardless of if there is data for each day
#levels function here is just adding 1, 2, 6, 7 (days) into the column with days of week
...
#Plot again
#scale_x_discrete(drop=FALSE) --> basically saying don't drop values that have 0
ggplot(sites,aes(x=dow)) +
  geom_bar() + 
...
#levels=c("1","2","3","4","5","6","7")) #Or... as.character(seq(1,7)) just wrangling them to be in order 
...
#good example of tidy code = break code across lines
sites$dow <- factor(
  sites$dow, 
