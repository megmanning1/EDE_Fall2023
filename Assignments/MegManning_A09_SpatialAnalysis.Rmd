...
> ANSWER: 4269 (NAD83). This is a geographic coordinate reference which uses the North America Datum from 1983. 
> 
...
>ANSWER: The lat/long fields disappeared and the new field created is geometry.
>
...
gage_sf_join <- usgs_gage.sf %>% 
  inner_join(gage_height, by = c("site_no")) 
...
ggplot() +
  geom_sf(data = counties_sf_utm, fill="beige") +
...
ggplot() +
  geom_sf(data = counties_sf_utm, fill="beige") +
  geom_sf(data = dawson_sf, fill = "maroon") +
  geom_sf(data = dawson_gages, color = "blue") +
  labs(title = "NWIS Gage Locations in Nebraska with Dawson Gage Sites",
       subtitle = "Meg Manning")
