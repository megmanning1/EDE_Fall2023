...
scrape.it <- function(the_year, PWSID){
  ...
  the_website <- read_html(paste0('https://www.ncwater.org/WUDC/app/LWSP/search.php', 
                                  PSWID, '/', the_year))
...
#FIRST ATTEMPT
#the_base_url <- 'https://www.ncwater.org/WUDC/app/LWSP/search.php'
#PWSID <- 03-32-010
#the_year <- 2015
#the_scrape_url <- paste0(the_base_url, '/', PWSID, '/', the_year)
#print(the_scrape_url)

#scrape.it <- function(the_year, PWSID){
  #the_scrape_url <- paste0(the_base_url, '/', PWSID, '/', the_year)
  
  #water_system <- webpage %>% html_nodes("div+ table tr:nth-child(1) td:nth-child(2)") %>% html_text()
  ...
  #MGD_bymonth <- webpage %>% html_nodes("th~ td+ td") %>% html_text()
...
durham_df <- scrape.it(2015, "03-32-010")
view(durham_df)

#plot the max daily withdrawals for Durham
ggplot(durham_df,aes(x=Date,y=Max_Day_Use)) + 
  geom_line() + 
  labs(title = paste(the_year, "Water usage data for Durham"),
       subtitle = PWSID,
       y="Withdrawal (mgd)",
       x="Date")
...
#8 Extract data for Asheville in 2015 SOMETHING'S WRONG WITH FUNCTION (water system name isn't changing :/)
the_year = 2015
PWSID = '01-11-010'
...
asheville_df <- lapply(X = the_year,
                  FUN = scrape.it,
                  PWSID=PWSID)
