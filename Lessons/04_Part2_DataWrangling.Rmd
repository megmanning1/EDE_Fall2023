...
#Even when we've already done some of this wrangling - it's important to run these functions to check out data
...
#double check our date - we'll have to change it again because when it's reimported the date will be a character
...
#use the filter() function to look at just nutrient data with == and |
...
#if you want to remove the other lakes that are showing 0 then use droplevels() to drop the 0s
...
#use the as.Date function to change date from factor to date again
NTL.nutrient.data.PeterPaul$sampledate <- as.Date(NTL.nutrient.data.PeterPaul$sampledate, format = "%m/%d/%y")
class(NTL.nutrient.data.PeterPaul$sampledate)

#we're going to do all other data processes with the pipe function 
#remember %>% - "then"
#start with dataset
#use mutate to add month column
#select the columns you're looking at 
#drop the N/A values
...
#QUESTION --> Mine didn't drop NAs from other columns? DO you have to specify drop_na for everything if you want that??
...
#use the select() function to take what columns you want
NTL.phys.data.PeterPaul.subset <- select(NTL.phys.data.PeterPaul, 
                                         lakename:irradianceDeck)
 
#Remove lakeID by using select() - another way to select columms you want by using commas  
...
#need to transform the 4 columns into 2 columns 
#assign a new variable then use pivot_longer()
#specify all of the columns NTL.nutrient.data.PeterPaul.subset,tn_ug:po4
#all nutrients should be spread into 2 columns 1) names_to = "nutrient" and 2) values_to = "concentration"
#taking all nutrient columns and turning into 2 columns 1 with category and 1 with nutrients 

#QUESTION _ CONCEPTUALLY CONFUSED ON WHAT'S HAPPENING?
...
#remove the NAs from the columns we don't want them in
#use subset(new dataframe), check if there is an na using !is.na(value we're looking at))
NTL.nutrient.data.PeterPaul.gathered <- subset(NTL.nutrient.data.PeterPaul.gathered, !is.na(concentration))
#check if first row is an na
#FALSE = not an NA
is.na(NTL.nutrient.data.PeterPaul.gathered$concentration[1])
is.na(NTL.nutrient.data.PeterPaul.gathered$concentration[2])
#count for each nutrient, how many observations do we have - dataset we're working with, nutrient means count by nutrient
...
#transforming rows and columns to make dataset wider than it was
#take data we just gathered and create the 2 columns we made before to transform back into 5 columns (take the 5 different values of nutrients and give them their own column with concentrations)
#opposite transformation than pivot_longer
...
#separate() - inform the data frame, then column you want to separate, then provide format for how to separate (c("Y", "m", "d")) - year, month, date
...
```{r joining data}
#use full join function - always a way to start if don't know
...
rbind will combine data by rows
...
```{r using rbind}
#first import all the data we want
...
#need to specify a path where all the files are located and specify pattern (which ones do you want to read ".csv")
LitterFiles = list.files(path = "./Data/Raw/NIWO_Litter/", pattern="*.csv", full.names=TRUE)
LitterFiles #has information on all 20 data sets - copied to an object all of the file names we want to read

#now we want to use a pipe (specify an order of functions/commands to perform) to read each element on the file using the read.csv
#if get an error - remember to load the correct packages!
Litter <- LitterFiles %>%
  plyr::ldply(read.csv)
```
...
#use left_join to include the three columns we want to add
LitterTrap <- left_join(Litter, Trap, by = c("siteID", "plotID", "trapID"))

#check the dimensions to see if/where we can join data
dim(Litter)
dim(Trap)
dim(LitterTrap)

#there are a lot of columns that we don't need so we'll use the select() function with the pipe notation to pick out what we want
...
#One example of using pipe notation to group_by
#assign an object
#first thing on pipeline - filter by depth = 0 
#group_by the lake name and the month 
#filter out na's in three columns (uses & to say where all three of those rows have actual elements)
#then summarize the mean of temp, the standard deviation of temp
