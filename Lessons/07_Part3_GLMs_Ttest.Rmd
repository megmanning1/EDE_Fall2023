...
Are Ozone levels below the threshold for "good" AQI index (0-50)? (is the mean less than 50?)**
...
length(EPAair$Ozone) #8976 observations - mean of our sample is a good representation of our population

#check for mu = 50, alternative = 'less' beacuse this is how you stated research question
O3.onesample <- t.test(EPAair$Ozone, mu = 50, alternative = "less")
O3.onesample

#reject the null - accepting that the population mean is <= 50 

#let's see results in a plot - asking for density plot, adding vertical line
...
```{r}
#First let's look at the data - hard to tell if the means are same or not
...
#reject null - that the two means are the same, true difference in means between group 2018 and group 2019 is not equal to 0 

# Format as a GLM - is year a predictor of ozone levels?
O3.twosample2 <- lm(EPAair$Ozone ~ EPAair$Year)
summary(O3.twosample2)

#year is indeed a predictor of ozone - the estimate value for EPAair$year with a p-value less than 0.05, we can reject the null and say that this variable is an indicator of whatever we are comparing it to
```
