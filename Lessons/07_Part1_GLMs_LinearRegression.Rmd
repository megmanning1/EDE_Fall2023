...
---
####See changes in header for editor_options: 
  chunk_output_type: console
...
The linear regression will test the null hypotheses that (no relationship where slope and intercept are zero)
...
## Simple Linear Regression (one variable on right side of equation)
...
#irradiance = light level based on depth 
#use function lm (linear model) - provide with y variable (irradiance) and x (independent variable, depth)
...
#another way is to provide dataframe so you don't have to use $
...
>  Can explain explain the correlation between irradiance and depth using different descriptive stats like residual standard error, r^2, p-value, etc. 
Significant negative correlation (lower light levels at higher depths)
...
2. **Normal Q-Q.** The points should fall close to the 1:1 line. We often see departures from 1:1 at the high and low ends of the dataset, which could be outliers. (normal quantile-quantile plot - will compare residuals to normal distribution)
...
#dividing plot screen into 4 cells, transforming screen into a grid
...
```{r, fig.height = 3, fig.width = 4}
# Plot the regression
#ylim() isn't changing data just zooming in where values are - disregarding outlier
irradiancebydepth <- 
...
```{r, fig.height = 3, fig.width = 4}
#taking log of data because linear model didn't fit, but eliminating 0s and outlier from data 
...
#saying margins are too small what is a good value??
par(mfrow = c(2,2), mar=c(4,4,4,4))
plot(irradiance.regression2)
par(mfrow = c(1,1))

# Add a line and standard error for the linear regression
# scale_y_log10() (y access is what is being logged)
irradiancebydepth2 <- 
  ggplot(PeterPaul.chem.nutrients, aes(x = depth, y = irradianceWater)) +
...
## Multiple Linear Regression (more than one variable on right side - more than one x)
...
``` {r, fig.height = 3, fig.width = 4}
#using lm and calling subset of data to only have obs for Paul lake 
TPregression <- lm(data = subset(PeterPaul.chem.nutrients, lakename == "Paul Lake"), 
                   tp_ug ~ depth + dissolvedOxygen)
summary(TPregression)

#p-value is less than 0.05 so there is some kind of correlation between variables (can reject null)
...
--> find relavent variables to add to graph, maybe create correlation matrix 
...
#correlation plot matrix - color and shape of ellipses give strength and kind of correlation (-1, 1 are strong correlation, and negative or positive)
...
Paul.naomit <- PeterPaul.chem.nutrients %>%
  ...
  na.omit()
#considering 5 columns of dataset tp_ug ~ depth + dissolvedOxygen + temperature_C + tn_ug + po4 (saying that these 5 variables are function of phosphorus)
TPAIC <- lm(data = Paul.naomit, tp_ug ~ depth + dissolvedOxygen + 
              temperature_C + tn_ug + po4)

#Choose a model by AIC in a Stepwise Algorithm
step(TPAIC) #call means those are the best variables to consider, the ones that will give best model
