...
-   **Autocorrelation**: Data points are not independent from one another (i.e., the measurement at a given time point is dependent on previous time point(s))
    ...
    -   Autocorrelation function (ACF) - **autocovariance and autocorrelation function give information about the dependence structure of a time series** (depending on result - it could mean that obs t is dependent on obs t-1, previous time is important to what is happening)
    -   **Partial autocorrelation (PACF)** - measures the linear dependency among the process variables Yt and Yt-h, basically think imagine if you could *remove* the influence of all of these intermediate variables you would only have direct correlation between Yt and Yt-h
    -   ACF and PACF will be built before fitting a model
    -   **Plotting ACF is a way to see seasonality in dataset**
...
**General Flow: 4 Components of TS** *(see decomposing a time series)*

-   **Trend**

-   **Seasonality** - maybe there's a difference between summer and winter, whenever it's associated with calendar events = seasonal

-   **Cycle** - can have up and down movements, not going to repeated every day/year, will be longer - not related to calendar events (usually represented with the trend and modeled together)

-   **Random** - we cannot model the random component of time series; think about the "noise" with the model (plot should be random, there shouldn't be a trend - mean should be close to zero)
...
**1) First thing to do in TMA -- plot data**
...
2) Let's start by **removing the NAs and splitting the dataset into the early and late years.**
...
```{r}
#only accepts argument that is a time series (ts)
#choosing columm we want to analyze (discharge mean) - specify start date and frequency (365 days a year)
...
```{r}
head(wind_data)
summary(wind_data$AWND) #helpful to find missing data, AWND is average wind

# Adding new column with no missing obs, just for illustration purpose
# In real applications you will simply replace NAs (can't use omit or drop nas because it's in the middle of the dataset, if it was first or last we could)
wind_data_clean <- 
  wind_data %>% 
...
#using month function to define 1st month and year
...
> Answer: Based on the statistical tests, we can conclude that these data are seasonal - ran the mann-kendall with low p-value meaning we reject the null, therefore there is a trend, and then using the smk.test, the overall trend is decreasing (- slope)
