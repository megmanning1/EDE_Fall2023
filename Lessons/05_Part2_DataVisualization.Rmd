...
#changing date to a date not a factor
...
#axis are black background is white
O3plot2 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) +
  theme_bw()
print(O3plot2)

#gray lines and axis background white
O3plot3 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) +
  theme_light()
print(O3plot3)

#no grid lines (background is white, lines are black)
...
```{r, fig.height = 3, fig.width = 4, warning = FALSE}
#define own theme
# based on theme_classic but changing font size for labels and change text elements
mytheme <- theme_classic(base_size = 14) +
  theme(axis.text = element_text(color = "black"), 
        legend.position = "top") #alternative: legend.position + legend.justification (alignment: right, center, left)
...
theme_set(mytheme) #set theme for all subsequent plots before running other plots and then you don't have to include theme in subsequent plots
...
O3plot7 <- ggplot(EPAair, aes(x = Date, y = Ozone)) +
  ...
  scale_x_date(limits = as.Date(c("2018-01-01", "2019-12-31")), 
    date_breaks = "2 months", date_labels = "%b %y") + #%b means abr. format of month
  ylab(expression("O"[3]* " AQI Value")) + #expression() means subscript
...
NvsP2 <-
  ...
  labs(color = "Depth", shape = "Lake Name", y = "Concentration in Micrograms per liter") + # change your legend labels here
...
# remove your x axis label - HOW DO YOU DO THIS?
...
library(cowplot)
plot_grid(NvsP2, Nutrientplot, nrow = 2, align = 'h', rel_heights = c(1.25, 1)) #rel_heights is relative heights and resizes the plots to fit on same page
