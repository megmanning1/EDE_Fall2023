...
#Create a map
mapView(huc8_sf_un, zcol = "SUBBASIN")

#layer both on ggplot - add them to the *geoms* NOT within the ggplot()
#color = is the outline of the shape
#fill = is filling the whole shape 
ggplot() +
  geom_sf(data = huc8_sf,fill ='orange') + 
  geom_sf(data = huc8_sf_un,fill='purple', shape = "SUBBASIN", color = "black") 

#, shape = "SUBBASIN" is calling out the subbasin and color = is the outline 

#layer both on in mapview
#alpha may control transparency
mapView(huc8_sf, col.regions = "orange") +
  mapView(huc8_sf_un, col.regions = "purple")

#change name of legend with layer.name = 
mapView(huc8_sf, col.regions = "orange", layer.name = "HUC 8") +
  mapView(huc8_sf_un, col.regions = "purple", layer.name = "Upper Neuse")

#Option to use | instead of + --> creates a swiper view map instead of just layered
mapView(huc8_sf, col.regions = "orange") |
  mapView(huc8_sf_un, col.regions = "purple")
```
...
#colClasses --> used to treat things like HUC12 as factors and not strings
#substr() is substring of characters and take everything from the 1st character to the 8th character
...
#%in% allows you to make a vector of all the counties you want
  
#Grab the intersecting HUC_8s
tri_intersec <- 
st_intersects() #a selection
st_intersection() #takes two datasets and will give you intersection
